...
import com.github.vanity.CacheLibraryMode;
import com.github.vanity.cache.index.archive.Archive;
import com.github.vanity.cache.index.archive.ArchiveSector;
import com.github.vanity.io.impl.InputStream;
import com.github.vanity.io.impl.OutputStream;
import com.github.vanity.progress.ProgressListener;
import com.github.vanity.utilities.*;
import com.github.vanity.utilities.Compression.CompressionType;
	...
	public ArchiveSector readArchiveSector(int id) {
		synchronized (super.origin.getMainFile()) {
			try {
				...
				if (super.origin.getMainFile().size() < ((long) Constants.INDEX_SIZE * id + Constants.INDEX_SIZE)) {
	...
	public boolean writeArchiveSector(int id, byte[] data) {
		synchronized (super.origin.getMainFile()) {
			try {
				...
				final byte[] buffer = new byte[Constants.ARCHIVE_SIZE];
				if (overWrite) {
					if ((long) Constants.INDEX_SIZE * id + Constants.INDEX_SIZE > randomAccessFile.size()) {
						return false;
					}
				...
				while (written < data.length) {
					...
					FileChannelUtil.writeData(super.origin.getMainFile(), written, length, data);
